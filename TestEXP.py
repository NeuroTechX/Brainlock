...
expName = 'TestEXP'  # from the Builder filename that created this script
...
# Initialize components for Routine "Password_Collect"
Password_CollectClock = core.Clock()
Instructions = visual.TextStim(win=win, ori=0, name='Instructions',
    text=u'Please \nEnter Password',    font=u'Arial',
...
#------Prepare to start Routine "Password_Collect"-------
t = 0
Password_CollectClock.reset()  # clock 
frameN = -1
# update component parameters for each repeat
StartEXP = event.BuilderKeyResponse()  # create an object of type KeyResponse
StartEXP.status = NOT_STARTED
# keep track of which components have finished
Password_CollectComponents = []
Password_CollectComponents.append(Instructions)
Password_CollectComponents.append(StartEXP)
for thisComponent in Password_CollectComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "Password_Collect"-------
continueRoutine = True
while continueRoutine:
    # get current time
    t = Password_CollectClock.getTime()
    ...
    if t >= 0.0 and StartEXP.status == NOT_STARTED:
        # keep track of start time/frame for later
        StartEXP.tStart = t  # underestimates by a little under one frame
        StartEXP.frameNStart = frameN  # exact frame index
        StartEXP.status = STARTED
        # keyboard checking is just starting
    if StartEXP.status == STARTED:
...
#-------Ending Routine "Password_Collect"-------
for thisComponent in Password_CollectComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "Password_Collect" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *Stim* updates
        if t >= 0.1 and Stim.status == NOT_STARTED:
            # keep track of start time/frame for later
            Stim.tStart = t  # underestimates by a little under one frame
            Stim.frameNStart = frameN  # exact frame index
            Stim.setAutoDraw(True)
        if Stim.status == STARTED and t >= (0.1 + (1-win.monitorFramePeriod*0.75)): #most of one frame period left
    ...
    # keep track of which components have finished
    isiComponents = []
    isiComponents.append(Fixation_Cross)
